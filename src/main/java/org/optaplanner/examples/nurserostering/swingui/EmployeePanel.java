...
import org.optaplanner.examples.nurserostering.domain.CalculateHolidays;
...
public class EmployeePanel extends JPanel
{
    ...
    {
        //check if we get here after using editDialogBox, WaalbrugWire, 6-5-2018
        //JOptionPane.showMessageDialog(null, "resetting Shiftlist panel");
        //yes it does reset a lot so there must be a problem with finding key for dayOffRequest 
        //shiftDates have different ids here so we are pushing the wrong shiftDate on the maps and lists
        //or the shiftList changes or gets copied???
        ...
        shiftDatePanelMap = new LinkedHashMap<>(shiftDateList.size() + 30);
        for (ShiftDate shiftDate : shiftDateList)
        {
            JPanel shiftDatePanel = new JPanel(new GridLayout(1, 0));

            //added JOptionPane for debugging  WaalbrugWire, 21-04-2018
            if (weekendDefinition == null)
            {
                JOptionPane.showMessageDialog(null, "weekendDefinition is empty");
            }
             ...
             */
            CalculateHolidays calcHoliday = new CalculateHolidays();
            ArrayList<LocalDate> holidays = new ArrayList<>();

            int temp = shiftList.get(shiftList.size() - 1).getShiftDate().getDate().getYear() - shiftList.get(0).getShiftDate().getDate().getYear();

            for (int i = 0; i < temp + 1; i++)
            {
                calcHoliday.calculateHolidays(holidays, shiftList.get(0).getShiftDate().getDate().getYear() + i);
            }
            
            Color backgroundColor;
            if (holidays.contains(shiftDate.getDate()))
            {
                backgroundColor = Color.lightGray;
            }
            else
            {
                backgroundColor = weekendDefinition.isWeekend(shiftDate.getDayOfWeek())
                    ? TangoColorFactory.ALUMINIUM_2 : shiftDatePanel.getBackground();
            }
        ...
        for (Shift shift : shiftList)
        {
            ...
            //added JOptionPane for debugging  WaalbrugWire, ??-??-2018
            if (shiftDatePanel == null)
            {
                JOptionPane.showMessageDialog(null, "shiftDatePanel is empty");
            }
            //runtime error is indeed caused by empty shiftDatePanel
            //end of debug message
            ...
            //changed tooltiptext from getLabel to getLongLabel to include year, WaalbrugWire, 5-5-2018           
            shiftPanel.setToolTipText("<html>Date: " + shift.getShiftDate().getLongLabel() + "<br/>"
    ...
    public void addShiftAssignment(ShiftAssignment shiftAssignment)
    {
        ...
        //added message box for debugging purposes, waalbrugwire 23-2-2018
        if (shift == null)
        {
            JOptionPane.showMessageDialog(null, "Shift is empty");
        }
        JPanel shiftPanel = shiftPanelMap.get(shift);
        //added message box for debugging purposes, waalbrugwire 23-2-2018
        if (shiftPanel == null)
        {
            JOptionPane.showMessageDialog(null, "ShiftPanel is empty, shiftPanel Map size is : " + shiftPanelMap.size());
        }
